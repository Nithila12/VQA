...
class PhraseLevel(nn.Module):
    ...
    def forward(self, X, form='ncl'):
        ...
        be = self.bigram(X)
        te = self.trigram(X)
        # Stack Nx20 Nx20 Nx20 => max(3xNx20) => [Nx20]
...
class AttentionNet(nn.Module):
    def __init__(self, nH):
        super().__init__()
        self.Wb = torch.nn.Parameter(
            torch.autograd.Variable(torch.randn(nH, nH)))
...
class ParallelAttention(nn.Module):
    def __init__(self, nH=512):
        ...
        self.Wv = torch.nn.Parameter(
            torch.autograd.Variable(torch.randn(nH, nH)))
        self.whv = torch.nn.Parameter(
            torch.autograd.Variable(torch.randn(1, nH)))
        self.whq = torch.nn.Parameter(
            torch.autograd.Variable(torch.randn(1, nH)))
...
class CoattentionNet(nn.Module):
    ...
    def forward(self, V, Q):
        ...
        Vphat, Qphat = self.parallelattention(V, Qpl)  # Nx
