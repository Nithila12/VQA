...
if __name__ == '__main__':
    ...
    ResNet = ResNet.cuda()  # Nx1000
    newmodel = torch.nn.Sequential(
        *(list(ResNet.children())[:-2]))  # Nx512x14x14
    ...
    traindata = DataLoader(train_D, batch_size=32,
                           shuffle=False, num_workers=10)
    validdata = DataLoader(val_D, batch_size=32, shuffle=False, num_workers=10)
    path = './student_code/supportfiles/ResNetData/'
    pathval = './student_code/supportfiles/ResNetDataVal/'
    ...
    for i, bdata in enumerate(tqdm(validdata)):
        ...
        for ii, kk in enumerate(k):
            ...
            np.save(nm, outs[ii].cpu().detach().numpy())
    print("Done")
    if not os.path.isfile('./student_code/supportfiles/GloVe300.d'):
        makeGloVe()
    if not os.path.isfile('./student_code/supportfiles/CoAttAns.d'):
        with open('./student_code/supportfiles/CoAttAns.d', 'wb') as file:
